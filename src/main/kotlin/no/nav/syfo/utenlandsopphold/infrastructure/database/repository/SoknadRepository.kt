...
class SoknadRepository(
    ...
    override fun lagreBehandlingsutfall(
        ...
        val behandlingsutfall =
            checkNotNull(soknadMedBehandlingsutfall.behandlingsutfall) {
                "Søknad ${soknadMedBehandlingsutfall.id} mangler behandlingsutfall etter fattVedtak/henlegg"
            }
    ...
    private fun Connection.toSoknader(pSoknader: List<PSoknad>): List<Soknad> {
        ...
        val vedtakPerioderPerBehandlingsutfall =
            getVedtakPerioder(behandlingsutfallPerSoknad.values.map { it.id })
                .groupBy { it.vedtakId }
        ...
        return pSoknader.map { pSoknad ->
            ...
            pSoknad.toSoknad(
                ...
                vedtakPerioder = pBehandlingsutfall?.let { vedtakPerioderPerBehandlingsutfall[it.id] }.orEmpty(),
            )
        }
    }
    ...
    private fun Connection.getVedtakPerioder(behandlingsutfallIds: List<Int>): List<PVedtakPeriode> {
    ...
    private fun Connection.lagreBehandlingsutfall(
        ...
        val pBehandlingsutfall =
            prepareStatement(CREATE_BEHANDLINGSUTFALL).use {
                ...
                it.setString(2, behandlingsutfall.dbTypeValue())
        ...
        val innvilgedePerioder = (behandlingsutfall as? Vedtak)?.innvilgedePerioder.orEmpty()
        createVedtakPerioder(pBehandlingsutfall.id, innvilgedePerioder)
    }
        ...
        private const val GET_IKKE_JOURNALFORTE_SOKNADER =
            """
                SELECT DISTINCT s.* FROM soknad s
                    INNER JOIN behandlingsutfall v ON v.soknad_id = s.id
